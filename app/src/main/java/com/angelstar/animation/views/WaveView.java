...
public class WaveView extends View {
     ...
     * +------------------------+__|____
     */
    public static final int DEFAULT_VIEW_SIZE = 100;
    ...
    private int mHeight;
    private int mWidth;
    private int minimumHeight = DEFAULT_VIEW_SIZE;
    private int minimumWidth = DEFAULT_VIEW_SIZE;
    private boolean mProportion;
    ...
    /**
     * Set whether the ratio, etc
     */
    public void setProportion(boolean mProportion) {
        ...
        if (mShowWave && mWaveShader != null) {
            ...
            switch (mShapeType) {
                ...
                case SQUARE:
                    ...
                    break;
                case HEART:
                    int divideWidth = mWidth / 10;
                    int divideHeight = mHeight / 10;

                    //Right
                    float rStartX = mWidth / 2;
                    float rStartY = divideHeight;

                    float rOneX = mWidth - divideWidth * 1.5f;
                    float rOneY = -divideHeight * 2.5f;

                    float rTwoX = mWidth + divideWidth * 4;
                    float rTwoY = divideHeight * 6;

                    float rThreeX = mWidth / 2;
                    float rThreeY = mHeight;

                    if (mPath == null) {
                        mPath = new Path();
                    }
                    mPath.reset();
                    mPath.moveTo(rStartX, rStartY);
                    mPath.cubicTo(rOneX, rOneY, rTwoX, rTwoY, rThreeX, rThreeY);
                    canvas.drawPath(mPath, mViewPaint);
                    canvas.drawPath(mPath, mBorderPaint);

                    //Left
                    float lStartX = mWidth / 2;
                    float lStartY = divideHeight;

                    float lOneX = divideWidth * 1.5f;
                    float lOneY = -divideHeight * 2.5f;

                    float lTwoX = -divideWidth * 4;
                    float lTwoY = divideHeight * 6;

                    float lThreeX = mWidth / 2;
                    float lThreeY = mHeight;
                    mPath.reset();
                    mPath.moveTo(lStartX, lStartY);
                    mPath.cubicTo(lOneX, lOneY, lTwoX, lTwoY, lThreeX, lThreeY);
                    canvas.drawPath(mPath, mViewPaint);
                    canvas.drawPath(mPath, mBorderPaint);
                    break;
            }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        compareWidth = Math.max(minimumWidth, widthSize);
        compareHeight = Math.max(minimumHeight, heightSize);
        if (widthMode == MeasureSpec.AT_MOST && heightMode == MeasureSpec.AT_MOST) {
            setMeasuredDimension(DEFAULT_VIEW_SIZE, DEFAULT_VIEW_SIZE);
        } else if (widthMode == MeasureSpec.AT_MOST) {
            setMeasuredDimension(DEFAULT_VIEW_SIZE, compareHeight);
        } else if (heightMode == MeasureSpec.AT_MOST) {
            setMeasuredDimension(compareWidth, DEFAULT_VIEW_SIZE);
        } else {
            setMeasuredDimension(compareWidth, compareHeight);
        }

        if ((mShapeType == ShapeType.HEART && mProportion) || (mShapeType == ShapeType.CIRCLE && mProportion)) {
